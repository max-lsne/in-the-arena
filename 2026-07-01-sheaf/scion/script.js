...
(function () {
  ...
  function load() {
    try {
      ...
      if (!parsed || !Array.isArray(parsed.scions)) return clone(SEED);
      parsed.scions = parsed.scions.slice(0, CAP).map(function (s) {
        return {
          id: s.id || id(),
  ...
  $count.addEventListener('dblclick', function () {
    if (window.confirm('Clear the bench back to the example nursery?')) {
      state = clone(SEED);
      selected = 0;
      render();
    }
