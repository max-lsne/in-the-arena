...
(function () {
  ...
  // Motion preference — a reader who asks for less motion gets no rising fill. Fill then steps
  // the load one notch at a time instead of pouring it. Re-checked live, so toggling the system
  ...
  function run() {
    if (motionOff()) { stepLoad(); return; }   // one notch, no rising fill
  ...
  // if the reader turns on reduced motion mid-sweep, stop the running fill at once
  if (reduceMQ) {
  ...
  document.addEventListener("keydown", function (e) {
    ...
    switch (e.key) {
      case "ArrowRight": if (isRange) return; setLoad(load + STEP_P); break;   // fill by hand
      case "ArrowLeft":  if (isRange) return; setLoad(load - STEP_P); break;   // empty by hand
      case "]": nudge(preRange, 6, PRE_MIN, PRE_MAX); break;                   // drive harder
      case "[": nudge(preRange, -6, PRE_MIN, PRE_MAX); break;                  // drive softer
      case "=": case "+": nudge(phiRange, 3, 15, 60); break;                   // stiffer hoop
      case "-": case "_": nudge(phiRange, -3, 15, 60); break;                  // more giving hoop
