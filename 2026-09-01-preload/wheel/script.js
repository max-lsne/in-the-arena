...
(function () {
  ...
  // Motion preference — a reader who asks for less motion gets no growing load. Load then steps
  // the weight one notch at a time instead of ramping it. Re-checked live, so toggling the
  // system setting takes effect without a reload, and halts a sweep already running.
  ...
  function run() {
    if (motionOff()) { stepLoad(); return; }   // one notch, no growing load
  ...
  // if the reader turns on reduced motion mid-sweep, stop the running load at once
  if (reduceMQ) {
  ...
  document.addEventListener("keydown", function (e) {
    ...
    switch (e.key) {
      case "ArrowRight": if (isRange) return; setLoad(load + STEP_P); break;   // load by hand
      case "ArrowLeft":  if (isRange) return; setLoad(load - STEP_P); break;   // lighten by hand
      case "]": nudge(preRange, 6, PRE_MIN, PRE_MAX); break;                   // shrink harder
      case "[": nudge(preRange, -6, PRE_MIN, PRE_MAX); break;                  // shrink softer
      case "=": case "+": nudge(phiRange, 3, 15, 60); break;                   // more giving spoke
      case "-": case "_": nudge(phiRange, -3, 15, 60); break;                  // stiffer spoke
