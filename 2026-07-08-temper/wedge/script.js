...
(function () {
  ...
  function load() {
    try {
      ...
      if (!parsed || !Array.isArray(parsed.balls)) return clone(SEED);
      parsed.balls = parsed.balls.slice(0, CAP).map(function (s) {
        return {
          id: s.id || id(),
  ...
  // --- reset: double-click the count to clear the board back to the example ---
  $count.addEventListener('dblclick', function () {
    if (window.confirm('Clear the board back to the example balls?')) {
      state = clone(SEED);
      selected = 0;
      render();
    }
  ...
  // --- keyboard: the board is worked, not clicked ---
  // N set a ball · J/K walk the balls · ]/[ or Enter read the body on/back
  // 0–3 move it along the wedging · R or Del throw it and take it off
  document.addEventListener('keydown', function (e) {
    ...
    switch (e.key) {
      ...
      case 'r': case 'R': case 'Delete': case 'Backspace':
        if (n) throwOff(selected); e.preventDefault(); break;
    }
  });

  function scrollToSel() {
    var li = $balls.querySelector('.sel');
