...
function sortedShown() {
  return visibleWithies().slice().sort((a, b) => {
    const d = STATES[a.state].order - STATES[b.state].order;
    if (d !== 0) return d;
    return b.winters - a.winters;
  });
...
function nudgeWinters(delta) {
  ...
  const s = withies.find(x => x.id === focused);
  if (!s) return;
...
function wireKeyboard() {
  document.addEventListener('keydown', (e) => {
    ...
    if (k === 'Enter') {
      if (focused != null) { e.preventDefault(); advanceWithy(focused); }
      return;
    }
    if (k === 'Delete' || k === 'Backspace') {
      if (focused != null) { e.preventDefault(); removeWithy(focused); focused = null; }
      return;
    }
    ...
    if (k === '2') { e.preventDefault(); setFilter('soaking');  return; }
    if (k === '3') { e.preventDefault(); setFilter('weaving');  return; }
    if (k === '4') { e.preventDefault(); setFilter('seasoned'); return; }
    if (k === '[') { e.preventDefault(); nudgeWinters(-1); return; }
    if (k === ']') { e.preventDefault(); nudgeWinters(1);  return; }
    if (k === 'r' || k === 'R') { e.preventDefault(); resetWithies(); return; }
  });
}
