/* Belay — a bench for the capstan equation, the law that lets one brake hand hold a falling
 * climber. Run the rope round the bends of a belay device and the tension in it bleeds away
 * exponentially round the arc of contact: whatever the fall brings on at one end, only a
 * fraction e^(−μθ) of it survives round the wrap to reach the brake hand. In the arena, 2026-09-04.
 *
...
(function () {
  ...
  var CFG = {
    storeKey: "arena.capstan.belay.v1",
    loadMin: 0, loadMax: 6000, loadNom: 2500,      // the climber's fall force, newtons
    handRef: 150,                                  // a single brake hand's grip, newtons
    turnsDefault: 1.5, muDefault: 0.35,
    actionOff: "Weight it",                        // press to bring the fall force on
    actionOn: "Ease off",                          // press again to let it off
    holdLabel: "brake",
    loadLabel: "fall",
    handNote: "one brake hand",
    stateFast: "held — one brake hand holds the fall",
    stateSurge: "slip — the rope runs; add wrap or friction"
  };
  ...
  // The belay device and its carabiner — the bend the rope is run round to brake it.
  function drawDrum() {
    add(make("circle", { cx: Cx, cy: Cy, r: Rd, class: "drum-body" }));
    // the gate: a chord across the carabiner, and its nose
    add(make("line", { x1: Cx - Rd * 0.66, y1: Cy - Rd * 0.4, x2: Cx + Rd * 0.5, y2: Cy + Rd * 0.62,
      class: "drum-cap", "stroke-width": 2 }));
    add(make("circle", { cx: Cx, cy: Cy, r: Rd * 0.34, class: "drum-cap" }));
    add(make("circle", { cx: Cx, cy: Cy, r: 3.2, class: "drum-centre" }));
    label(Cx, Cy + Rd + PAD + state.turns * GAP + 16, "middle", "device", "drum-label");
  }
