/* Bollard — a bench for the capstan equation, the law that lets a hand hold a ship. Lead a
 * line a few turns round a post and the tension in it bleeds away exponentially round the arc
 * of contact: whatever load comes on at one end, only a fraction e^(−μθ) of it survives round
 * the wrap to reach the hand. In the arena, 2026-09-04.
 *
...
(function () {
  ...
  var CFG = {
    storeKey: "arena.capstan.bollard.v1",
    loadMin: 0, loadMax: 80000, loadNom: 30000,   // the ship's pull, newtons
    handRef: 300,                                  // a single steady dockhand's pull, newtons
    turnsDefault: 3, muDefault: 0.25,
    actionOff: "Take the strain",                  // press to bring the load on
    actionOn: "Ease off",                          // press again to let it off
    holdLabel: "tail",
    loadLabel: "ship",
    handNote: "one steady pull",
    stateFast: "fast — one hand holds the ship",
    stateSurge: "surge — the tail cannot hold it; take another turn"
  };
  ...
  // The post seen from above — a stout disc the rope is taken round.
  function drawDrum() {
    add(make("circle", { cx: Cx, cy: Cy, r: Rd, class: "drum-body" }));
    add(make("circle", { cx: Cx, cy: Cy, r: Rd * 0.4, class: "drum-cap" }));
    add(make("circle", { cx: Cx, cy: Cy, r: 3.2, class: "drum-centre" }));
    label(Cx, Cy + Rd + PAD + state.turns * GAP + 16, "middle", "bollard", "drum-label");
  }
  ...
  function speak() {
    ...
    el.status.textContent =
      f1(state.turns) + " turns round the post — a wrap of " + Math.round(state.turns * 360) +
      " degrees — on a surface of mu " + state.mu.toFixed(2) + ". Friction multiplies the hold by " +
      Math.round(amp()) + " times, so the ship's pull of " + fmtF(state.load) +
      " arrives at the tail as only " + fmtF(hold()) + ". " +
      (surging()
        ? "That is more than one steady pull can hold — the line would surge; take another turn."
        : "One steady hand can hold that, up to a pull of " + fmtF(maxOneHand()) + ".");
  }
