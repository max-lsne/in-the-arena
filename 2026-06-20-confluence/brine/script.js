...
function sortedShown() {
  return visibleCrocks().slice().sort((a, b) => {
    const d = STATES[a.state].order - STATES[b.state].order;
    if (d !== 0) return d;
    return b.turns - a.turns;
  });
...
function nudgeTurns(delta) {
  ...
  const s = crocks.find(x => x.id === focused);
  if (!s) return;
...
function wireKeyboard() {
  document.addEventListener('keydown', (e) => {
    ...
    if (k === 'Enter') {
      if (focused != null) { e.preventDefault(); advanceCrock(focused); }
      return;
    }
    if (k === 'Delete' || k === 'Backspace') {
      if (focused != null) { e.preventDefault(); removeCrock(focused); focused = null; }
      return;
    }
    if (k === 'n' || k === 'N') { e.preventDefault(); const el = $('#add-name'); if (el) el.focus(); return; }
    if (k === '0') { e.preventDefault(); setFilter('all');     return; }
    if (k === '1') { e.preventDefault(); setFilter('fresh');   return; }
    if (k === '2') { e.preventDefault(); setFilter('salting'); return; }
    if (k === '3') { e.preventDefault(); setFilter('souring'); return; }
    if (k === '4') { e.preventDefault(); setFilter('cured');   return; }
    if (k === '[') { e.preventDefault(); nudgeTurns(-1); return; }
    if (k === ']') { e.preventDefault(); nudgeTurns(1);  return; }
    if (k === 'r' || k === 'R') { e.preventDefault(); resetCrocks(); return; }
  });
}
