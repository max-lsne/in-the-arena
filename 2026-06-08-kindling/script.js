...
function sortedShown() {
  return visibleSticks().slice().sort((a, b) => {
    const d = STATES[a.state].order - STATES[b.state].order;
    if (d !== 0) return d;
    return b.weeks - a.weeks;
  });
...
function nudgeWeeks(delta) {
  if (focused == null) return;
  const s = sticks.find(x => x.id === focused);
  if (!s) return;
  s.weeks = Math.max(0, s.weeks + delta);
  render();
}
...
function wireKeyboard() {
  document.addEventListener('keydown', (e) => {
    ...
    if (k === 'Enter') {
      if (focused != null) { e.preventDefault(); advanceStick(focused); }
      return;
    }
    if (k === 'Delete' || k === 'Backspace') {
      if (focused != null) { e.preventDefault(); removeStick(focused); focused = null; }
      return;
    }
    if (k === 'n' || k === 'N') { e.preventDefault(); const el = $('#add-name'); if (el) el.focus(); return; }
    if (k === '0') { e.preventDefault(); setFilter('all');      return; }
    if (k === '1') { e.preventDefault(); setFilter('gathered'); return; }
    if (k === '2') { e.preventDefault(); setFilter('drying');   return; }
    if (k === '3') { e.preventDefault(); setFilter('ready');    return; }
    if (k === '4') { e.preventDefault(); setFilter('spent');    return; }
    if (k === '[') { e.preventDefault(); nudgeWeeks(-1); return; }
    if (k === ']') { e.preventDefault(); nudgeWeeks(1);  return; }
    if (k === 'r' || k === 'R') { e.preventDefault(); resetSticks(); return; }
  });
}
