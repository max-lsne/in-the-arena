...
function sortedShown() {
  return visibleMarks().slice().sort((a, b) => {
    const d = STATES[a.state].order - STATES[b.state].order;
    if (d !== 0) return d;
    return b.turns - a.turns;
  });
...
function nudgeTurns(delta) {
  ...
  const s = marks.find(x => x.id === focused);
  if (!s) return;
...
function wireKeyboard() {
  document.addEventListener('keydown', (e) => {
    ...
    if (k === 'Enter') {
      if (focused != null) { e.preventDefault(); advanceMark(focused); }
      return;
    }
    if (k === 'Delete' || k === 'Backspace') {
      if (focused != null) { e.preventDefault(); removeMark(focused); focused = null; }
      return;
    }
    ...
    if (k === '4') { e.preventDefault(); setFilter('true');    return; }
    if (k === '[') { e.preventDefault(); nudgeTurns(-1); return; }
    if (k === ']') { e.preventDefault(); nudgeTurns(1);  return; }
    if (k === 'r' || k === 'R') { e.preventDefault(); resetMarks(); return; }
  });
}
