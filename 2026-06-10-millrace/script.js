...
function sortedShown() {
  return visibleChannels().slice().sort((a, b) => {
    const d = STATES[a.state].order - STATES[b.state].order;
    if (d !== 0) return d;
    return b.paddles - a.paddles;
  });
...
function nudgePaddles(delta) {
  if (focused == null) return;
  const s = channels.find(x => x.id === focused);
  if (!s) return;
  s.paddles = Math.max(0, s.paddles + delta);
  render();
}
...
function wireKeyboard() {
  document.addEventListener('keydown', (e) => {
    ...
    if (k === 'Enter') {
      if (focused != null) { e.preventDefault(); advanceChannel(focused); }
      return;
    }
    if (k === 'Delete' || k === 'Backspace') {
      if (focused != null) { e.preventDefault(); removeChannel(focused); focused = null; }
      return;
    }
    if (k === 'n' || k === 'N') { e.preventDefault(); const el = $('#add-name'); if (el) el.focus(); return; }
    if (k === '0') { e.preventDefault(); setFilter('all');    return; }
    if (k === '1') { e.preventDefault(); setFilter('spring'); return; }
    if (k === '2') { e.preventDefault(); setFilter('head');   return; }
    if (k === '3') { e.preventDefault(); setFilter('race');   return; }
    if (k === '4') { e.preventDefault(); setFilter('tail');   return; }
    if (k === '[') { e.preventDefault(); nudgePaddles(-1); return; }
    if (k === ']') { e.preventDefault(); nudgePaddles(1);  return; }
    if (k === 'r' || k === 'R') { e.preventDefault(); resetChannels(); return; }
  });
}
